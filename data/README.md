...
5. The parser automatically runs and updates:
   ...
   - **Note:** `/app/data/work.ts` is **manually curated** and NOT auto-generated by the LaTeX parser. Its `description` and `technologies` are AI-generated into `/app/data/role-content.json` (see [AI Role Content](#ai-role-content)).
...
Separate from the LaTeX parser, an AI step generates the prose/tech that the Work and
Experience sections display, keeping it in sync with the résumé bullets while leaving `work.ts`
hand-curated.
...
- **Output**: `/app/data/role-content.json` (committed to git, hand-editable)
- **Secret**: reads `GEMINI_API_KEY` from `.env` — this is the **only** command that reads the
  key or hits the network. `npm run dev`, `npm run build`, and Vercel never call it; they only
  read the committed JSON

### What It Generates

Per role, keyed by a stable `contentKey`:
...
the parser's keyword tech if no entry exists). `description` is Work-only — the Experience
ledger shows the real résumé bullets on expand, never an AI summary.

### Role Mapping

The role list is hand-maintained in `scripts/generate-role-content.ts`. Each row pins a
`contentKey` to an `experienceId` with a guard string (matched against the parsed role's
title/department/company) that fails loud if a résumé reorder shifts ids:

| `contentKey` | `experienceId` | Gets `description`? |
|--------------|----------------|---------------------|
| `doubl` | 1 | No (coming-soon card keeps manual copy) |
| `mds` | 2 | Yes |
| `makerspace` | 3 | Yes |
| `learncoding` | 4 | Yes |
| `teaching-assistant` | 5 | No (no Work card; tech shown in Experience only) |

A role wants a `description` iff a non-`comingSoon` `work.ts` item carries its `contentKey`.

### Approve / Lock Workflow

1. Run `npm run generate:content` — unapproved or bullet-changed roles get fresh drafts
   (`approved: false`); approved + unchanged roles are skipped (no API call)
2. Review `/app/data/role-content.json`; set `approved: true` on the good drafts (or hand-edit)
3. Re-run — approved roles are skipped and approvals are never lost; only unapproved ones
   redraft. Editing a role's bullets in the `.tex` changes its `sourceHash`, forcing a redraft
   that flips it back to `approved: false` for re-review
4. **Commit** `role-content.json` after approving, like the regenerated `app/data/*.ts`

### Flags

| Flag | Effect |
|------|--------|
| _(none)_ | Draft only unapproved or bullet-changed roles |
| `--force` | Redraft every role, even approved ones |
| `--force <contentKey>` | Redraft only the named role |
| `--seed` | Fill the file from current committed values (`approved: true`, correct hashes) with **no API call** — used to bootstrap the file so the first live run is a no-op |
