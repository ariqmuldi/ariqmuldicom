# app/data — site content & résumé data
...
3. **Replace** `/app/data/master-resume.tex` with the new file
...
5. The parser automatically runs and updates:
   ...
   - **Note:** `/app/data/work.ts` is **manually curated** and NOT auto-generated by the LaTeX parser. Its `description` and `technologies` are AI-generated into `/app/data/work-experience-content.json` (see [AI Role Content](#ai-role-content)).
...
- **Source**: Education section in `/app/data/master-resume.tex`
...
3. Download and replace `/app/data/master-resume.tex`
...
- **Source**: Projects section in `/app/data/master-resume.tex`
...
### Configuration File: `/app/data/resume-config.json`
...
1. Edit `/app/data/resume-config.json` to change visibility settings
...
2. Download and replace `/app/data/master-resume.tex`
...
This will:
- Read `/app/data/master-resume.tex`
- Read `/app/data/resume-config.json` (if it exists)
...
Because build skips parsing, the generated `app/data/*.ts` files are committed to the repo so
...
- **Commands**:
  ...
  - `npm run generate:work-experience-content` (`scripts/generate-work-experience-content.ts`) —
    writes `/app/data/work-experience-content.json`; takes flags
  - `npm run generate:project-content` (`scripts/generate-project-content.ts`) — writes
    `/app/data/project-content.json`; takes flags
...
- **Output**: `/app/data/work-experience-content.json` and `/app/data/project-content.json`
...
|--------------|----------------|---------------------|-----------------------|
| `doubl` | 1 | No (coming-soon card keeps manual copy) | No (no accomplishments) |
| `mds` | 2 | Yes | Yes |
...
1. Run `npm run generate:content` (or a specific generator) — unapproved or source-changed
   ...
   API call)
2. Review `/app/data/work-experience-content.json` and `/app/data/project-content.json`; set
...
4. **Commit** both JSON files after approving, like the regenerated `app/data/*.ts`
...
Download .tex file
    ↓
Replace app/data/master-resume.tex
    ↓
npm run dev
    ↓
