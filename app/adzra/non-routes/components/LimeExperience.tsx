...
// dir). Importing the images as modules lets webpack bundle them into hashed, unguessable URLs
// instead of exposing them at a predictable public path, keeping the /adzra route self-contained.
...
// The July 4 one-off ("For Sayang, Working in the UK") — the original /adzra lime experience,
// preserved exactly (framer-motion, the lime → lime-with-two-friends crossfade, encouragements,
// progress dots, drifting floaters). The only addition is a back arrow to the collection. The
// reveal note is passed in (from the entry data) so it stays a single source of truth.
export default function LimeExperience({ message }: { message: string }) {
	...
	return (
		<main className="relative flex min-h-[100dvh] w-full items-center justify-center bg-gradient-to-b from-[#FFF8EC] via-[#FFF8EC] to-[#DCCCAC] px-5 py-10 font-sans sm:py-12">
			...
			<Link
				href="/adzra"
				className="fixed left-4 top-4 z-30 flex items-center gap-1 rounded-full border border-[#546B41]/25 bg-[#FFF8EC]/80 px-3 py-1.5 text-sm text-[#546B41] backdrop-blur-sm transition-colors hover:bg-[#FFF8EC]"
			>
				‹ collection
			</Link>
			...
			<div className="relative z-10 w-full max-w-xl">
					...
					>
						{!revealed ? (
							...
							<>
								...
								<p className="mt-3 text-base text-[#546B41]/70 sm:text-lg">
									Look — it&rsquo;s lime with two friends, cheering you on
								</p>
							</>
						)}
					</motion.div>
				...
				<motion.button
					...
					aria-label={revealed ? 'A lime with two friends' : 'Click the lime for a surprise'}
				...
				<div className="mt-6 min-h-[4rem] text-center">
					{!revealed ? (
						...
						>
							<p className="whitespace-pre-line text-base leading-relaxed text-[#546B41] sm:text-lg">
								{message}
							</p>
						</motion.div>
					)}
